...
//   if (!string) return;
//   const size_t len = strlen(string); // cast it to PGM_P, which is basically const char *, and measure it using the _P version of strlen.
//   if (len == 0) return;
//   memcpy(&DWIN_SendBuf[i+1], string, len);
//   i += len;
// }
...
#define CHECK_CONNECTION(ser) CALL_IF_EXISTS(bool, static_cast<SerialT*>(ser), connected);
...
bool DWIN_Handshake(void) {
  ...
  LCD_SERIAL.begin(LCD_BAUDRATE, SERIAL_8N1, 16, 17);
  // const millis_t serial_connect_timeout = millis() + 1000UL;
  // while (!LCD_SERIAL.available() && PENDING(millis(), serial_connect_timeout)) { /*nada*/ }
  delay(10);
